...
// Data to seed
$oldTasksFile = '/var/clickforcharity-data/complex-tasks/tasks.json';
$tasksData = [];

if (file_exists($oldTasksFile)) {
    $tasksData = json_decode(file_get_contents($oldTasksFile), true);
    echo "Loaded " . count($tasksData) . " tasks from old file.<br>";
} else {
    // Default fallback if old file missing
    $tasksData = [
        [
            "id" => "task_001",
